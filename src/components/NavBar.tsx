...
export default function NavBar() {
  ...
  // Simulate user signed-in state. Replace with real auth logic as needed.
  const [isSignedIn, setIsSignedIn] = useState(false);

  useEffect(() => {
    if (typeof window !== "undefined") {
      setIsSignedIn(localStorage.getItem("wisker_signed_in") === "true");
    }
  }, []);

  const handleLogout = () => {
    if (typeof window !== "undefined") {
      localStorage.removeItem("wisker_signed_in");
      setIsSignedIn(false);
