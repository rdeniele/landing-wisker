...
function formatPrice(price: number) {
  return price === 0 ? "Free" : `₱${price.toLocaleString("en-PH", { maximumFractionDigits: 0 })}`;
}
...
const Pricing = () => {
  ...
  <section className="w-full flex flex-col items-center py-12 px-2 sm:px-4 rounded-3xl">
      ...
      <div className="mb-10 flex gap-2 bg-white rounded-xl shadow-sm p-1">
        ...
        >
          Yearly <span className="text-green-600 font-bold text-base ml-1">(-20%)</span>
        </button>
      </div>
      {/* Pricing Cards */}
      <div className="flex flex-col md:flex-row gap-6 justify-center w-full max-w-5xl">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.name}
              className={`flex-1 flex flex-col items-center rounded-2xl px-8 py-10 min-w-[240px] max-w-xs mx-auto relative border-4 transition-transform duration-200 ease-in-out
            ...
            >
              {plan.name === "Pro" && (
                <span className="absolute top-3 right-3 bg-indigo-700 text-white rounded-md px-3 py-1 text-xs font-bold shadow">Most Popular</span>
              )}
              <h3 className="text-xl font-bold mb-2 tracking-wide text-[#5c5c5c]">{plan.name}</h3>
              {/* Animated Price */}
              {/* 50% OFF badge for Pro plan, above price */}
              {plan.name === "Pro" && (
                <div className="mb-1 flex flex-col items-center w-full">
                  ...
                  {isYearly ? (
                    <span className="text-lg font-semibold text-red-500 line-through">₱{(99 * 12).toLocaleString('en-PH', { maximumFractionDigits: 0 })}</span>
                  ) : (
                    <span className="text-lg font-semibold text-red-500 line-through">₱99</span>
                  )}
                </div>
              )}
              <div
              ...
              >
                {plan.name === 'Pro'
                  ? isYearly
                    ? `₱${(99 * 12 * 0.5 * 0.8).toLocaleString('en-PH', { maximumFractionDigits: 0 })}`
                    : '₱50'
                  : formatPrice(price)}
                {plan.price !== 0 && (
                  <span className="text-base font-medium ml-1 text-[#5c5c5c]">/{per}</span>
                )}
              </div>
              {isYearly && plan.price !== 0 && (
                <div className="text-green-600 text-xs font-semibold mb-2">Save 20% with annual billing</div>
              )}
              <ul className="list-none p-0 my-4 w-full">
                {plan.features.map((feature) => (
                  <li key={feature} className="mb-2 text-[15px] font-medium text-[#5c5c5c]">• {feature}</li>
                ))}
              </ul>
              <button
      ...
      <div className="mt-10 text-gray-400 text-sm text-center max-w-xl font-medium">
        <strong className="text-gray-700">Launch Offer:</strong> All plans are <span className="text-green-600 font-bold">50% off</span> for a limited time!<br />
        <span>Get an extra <span className="text-green-600 font-bold">20% off</span> when you choose annual billing.</span>
      </div>
    </section>
  );
};
